...
static void processGPSData()
{
 ...
 if (gps.location.isValid())
 {
    ...
    if ((UGPS.Seconds <= 2) && (UGPS.Minutes % 2 == 0)) 
    {
      ...
      // Solo si es un minuto par (cada 2 minutos) y estamos al inicio del slot
if (UGPS.Seconds <= 2 && UGPS.Minutes % 2 == 0) {

  int index = (UGPS.Minutes % 20) / 2;
  ...
  freq = freqArray[index];
  UGPS.sendMsg1 = true;

  #if defined(DEVMODE)
    Serial.print("[WSPR] Minuto UTC: ");
    Serial.print(UGPS.Minutes);
    Serial.print(" → Índice: ");
    Serial.print(index);
    Serial.print(" → Frecuencia: ");
    Serial.print(freq);
    Serial.print(" Hz → Banda: ");
    Serial.println(bandLabels[index]);
  #endif
}
...
void Get_MaidenHead() {
  ...
  Serial.print("[GPS] Maidenhead calculado: ");
